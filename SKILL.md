...
All debug instrumentation **logs to a file** (`./debug-output.log` by default) rather than only to stdout or console. This lets the agent read the log file directly after reproduction instead of relying on the user to copy-paste output.
...
- At least one hypothesis should cover an assumption you are NOT sure about
...
- **Browser app** -> `fetch()` to the log ingest server. Start the server first. Do NOT substitute `console.log` because it seems easier or because you are confident in a hypothesis. The only valid reason to use `console.log` is if the user explicitly refused to run the ingest server.
...
This lets the agent grep the log file by hypothesis (for example, all `H2` lines) during analysis. Since all output goes to an isolated log file, no other prefix is needed.
...
This is a mechanical cross-reference, not a judgment call. Quote the specific log line(s) that match the Confirm or Rule-out criteria when writing the verdict.

- If any are `INCONCLUSIVE`, go back to Phase 2 with more targeted instrumentation for those specific active hypotheses
...
[If generating new hypotheses, this IS a Phase 1 cycle. You MUST list at least
3 new hypotheses here — not later, not in instrumentation comments. Each must
follow Phase 1 format. The same Phase 1 rules apply: structurally distinct
fault mechanisms, ordered by plausibility, at least one covering an assumption
you are not sure about. Do not proceed to Phase 2 until every new hypothesis
is written below.]
...
Choose this when the prior active hypotheses are no longer sufficient or all were ruled out by the latest evidence. **This is a full Phase 1 cycle** — the same rules and rigor apply.

Requirements:
- **Produce at least 3 new hypotheses immediately, in Phase 1 format** — each must have a short label, mechanism explanation, and confirming/refuting evidence criteria. An `Hn` label in an instrumentation comment is NOT a hypothesis — it is just a tag. The hypothesis must be defined before instrumentation can reference it.
- The hypotheses must be **structurally distinct** — do not list variations of the same theory about why the fix failed
- Start numbering at the next unused session-global label
- Keep any still-relevant older hypotheses by original label if you explicitly carry them forward; do not renumber them
- Return to Phase 2 for instrumentation of the new active hypotheses
